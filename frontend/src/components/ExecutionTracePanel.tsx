...
import { Layers, Activity, ChevronRight, CheckCircle2, AlertCircle } from 'lucide-react';
...
export const ExecutionTracePanel: React.FC<ExecutionTracePanelProps> = ({
  ...
  return (
    <aside
      ...
      aria-label="Execution Trace Panel"
    >
        ...
        <div className="trace-header-right">
          {overallStatus && (
            <span className={`status-badge badge-${overallStatus}`}>
              ...
              {overallStatus === 'partial_success' && <AlertCircle size={12} />}
              {overallStatus}
            </span>
          )}
          {onClose && (
