...
## 2. Core Philosophy (核心開發哲學)

1. **The developer must understand the code**（理解優先於單純跑通）：所有重要程式碼修改皆附帶新手友善的白話文解釋與架構說明。
2. **Protocol First**：外部能力與代理人溝通優先採用開放標準協定（MCP / A2A）。
3. **Incremental Development**：專案按 Phase 階段式推進，不提前引入未到期之複雜架構。
4. **Security & Permission**：任何外部操作皆設有明確權限邊界，機密資訊（API Keys）絕不提交。
